...
#            yearRange = '(%s)' % (','.join,)((lambda .0: for i in .0:"'%s'" % str(i))(range(thisYear - 1, thisYear + 1)))
            yearRange = '(%s)' % ','.join("'%s'" % i for i in range(thisYear - 1, thisYear + 1))
...
#            $append0 = [].append
#            del $append0
#            upcStr = []([ "'%s'" % u for u in upcList ])
            upcStr = ','.join("'%s'" % u for u in upcList)
            sql = 'SELECT movie_id,title,genre,movie_release_year,old_movie_id, directors, starring, pic_name, big_pic_name, rating, upc, dvd_release_date,dvd_version FROM upc WHERE upc IN (%s);' % WupcStr
            upcDb = Db(UPC_DB_PATH)
            rows = upcDb.query(sql)
#            $append0 = [].append
#            del $append0
