# Source Generated with Decompyle++Debug
...
#        $append0 = [].append
#        del $append0
#        []([], [ (itm[1], itm[0]) for itm in params.items() ])
        values = [(itm[1], itm[0]) for itm in params.items()] 
...
#        sql = f"SELECT upc, dvd_version FROM upc WHERE upc IN {upcstr};"
    ...
    def _filterBluray(self, upcs):
        ...
        blurays = []
        upcstr = '()'
        if type(upcs) in types.StringTypes:
            upcstr = "('%s')" % upcs
        elif type(upcs) == types.IntType:
            upcstr = "('%s')" % upcs
        elif type(upcs) in (types.ListType, types.TupleType):
#            upcstr = '(%s)' % (','.join,)((lambda .0: for upc in .0:"'%s'" % upc)(upcs))
    ...
    def getRemoteData(self, funcName, params, timeout = 60):
        ...
        '''
        self.log.error('XXX getRemoteData >>> 0 <<<')
        url = '/api'
    ...
    def _httpCall(self, url, params, timeout = 60):
        ...
        self.log.error('XXX _httpCall >>> 0 <<<')
        self.log.error(self.config['SYNC_HOST'])
        self.log.error(self.config['SYNC_PORT'])
        self.log.error('XXX _httpCall >>> 1 <<<')
        http = httplib.HTTPConnection(self.config['SYNC_HOST'], self.config['SYNC_PORT'])
        headers = {
    ...
    def _http_call(self, url, params, timeout = 60):
        '''Calls a remote function on a web server'''
        self.log.info('XXX _http_call >>> 0 <<<')
        headers = {
            'Content-type': 'application/x-www-form-urlencoded',
            'Accept': 'text/plain',
            'Kiosk': self.kioskId }
        ...
        try:
            self.log.info('XXX _http_call >>> 1 <<<')
            socket.setdefaulttimeout(timeout)
            self.log.info('XXX _http_call >>> 2 <<<')
            for i in range(2):
                request = urllib2.Request(url, urlParams, headers)
                opener = urllib2.build_opener(urllib2.HTTPCookieProcessor())
                urllib2.install_opener(opener)
                self.log.info('XXX _http_call >>> 3 <<<')
                data = opener.open(request).read()
                self.log.info('XXX _http_call >>> 4 <<<')
                try:
                    self.log.info('XXX _http_call >>> 5 <<<')
                    self.log.info(repr(data));
                    result = eval(data)
                    self.log.info('XXX _http_call >>> 6 <<<')
                except Exception:
                    self.log.info('XXX _http_call >>> EX <<<')
                    ex = None
            ...
            ex = None
            msg = str(ex)
            self.log.error('1httpCall exception: ' + msg)
            result = {
                'result': 'error',
                'zdata': 'connection error: %s' % msg }
        self.log.info('XXX _http_call >>> 7 <<<')
        self.log.info(repr(result));
        self.log.info('XXX _http_call >>> 8 <<<')
        return result
