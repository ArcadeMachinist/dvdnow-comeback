...
class UmsProxy(Proxy):
    ...
    def setMemberDetail(self, customer, shoppingCart):
        ...
        try:
            ...
            self.log.info('XXX setMemberDetail >>> 2 <<<')
            self.log.info('setMemberDetail %s: %s' % (customer.ccid, info.get('email', '')))
            if not info:
                info = { }
            
            email = info.get('email', '')
            self.log.info('XXX setMemberDetail >>> 3 <<<')
            age = self.getAge(info.get('birth_year', ''))
            self.log.info('XXX setMemberDetail >>> 4 <<<')
            memberRating = info.get('member_rating', '')
            self.log.info('XXX setMemberDetail >>> 5 <<<')
            memberID = info.get('member_id', '')
            self.log.info('XXX setMemberDetail >>> 6 <<<')
            firstName = info.get('first_name', '')
            self.log.info('XXX setMemberDetail >>> 7 <<<')
            lastName = info.get('last_name', '')
            self.log.info('XXX setMemberDetail >>> 8 <<<')
            cerepay = info.get('cerepay', { })
            self.log.info('XXX setMemberDetail >>> 9 <<<')
            primaryCcId = customer.ccid
            primaryCcNumber = customer.ccNum
            allCcIds = [
                customer.ccid]
            gender = info.get('gender', '')
            self.log.info('XXX setMemberDetail >>> A <<<')
            isMember = False
            if email:
                isMember = True
            ...
            self.log.info('XXX setMemberDetail >>> B <<<')
            self._setCustomerCerePayInfo(customer, cerepay)
            self.log.info('XXX setMemberDetail >>> C <<<')
    ...
    def getMemberCouponForKiosk(self, customer):
        ...
        try:
            ...
            for coupon in tmpcoupons.values():
                coupon['status'] = '0'
    ...
    def getCardInfoForCerePay(self, customer):
        ...
        try:
            ...
            if customer.cardType == 3:
                self.log.error('XXX getCardInfoForCerePay >>> 0 <<<')
                cpCfg = proxy.getCerePayCfg()
                if cpCfg:
                    cpInfo = proxy.getCerePayUserInfo(cpCfg['MERCHANTID'], cpCfg['PASSWORD'], cpCfg['CURRENCY'], number = customer.ccNum)
                    self.log.error('XXX getCardInfoForCerePay >>> 1 <<<')
                    errCode = str(cpInfo['errCode'])
                    self.log.error('XXX getCardInfoForCerePay >>> 2 <<<')
                    if errCode == '1004':
                        status = 0
                    elif errCode == '1007':
                        status = 3
